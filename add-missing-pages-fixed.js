...
async function addMissingPagesFixed() {
  try {
    console.log('🔧 Adding Missing Pages (Fixed)...');
    ...
    for (const missingPage of missingPages) {
      ...
      try {
        // Get page details with correct fields
        const pageResponse = await fetch(`https://graph.facebook.com/v18.0/${missingPage.id}?fields=id,name,category,category_list&access_token=${credentials.accessToken}`);
        ...
        console.log(`✅ Page: ${pageData.name} (Category: ${pageData.category})`);
        ...
        if (instagramData.instagram_business_account) {
          ...
          console.log(`ℹ️  No Instagram business account linked`);
        }

        // Create page object with correct structure
        const pageObject = {
          id: pageData.id,
          name: pageData.name,
          category: pageData.category,
          category_list: pageData.category_list,
          tasks: ['MODERATE', 'MESSAGING', 'ANALYZE', 'ADVERTISE', 'CREATE_CONTENT', 'MANAGE'], // Default tasks
