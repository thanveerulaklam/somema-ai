  ...
  try {
    ...
    const analytics = {
      ...
      recentPayments: payments.slice(0, 10).map(payment => ({
        ...
        plan: payment.payment_orders.plan_id,
        customer: payment.payment_orders.user_profiles.business_name || payment.payment_orders.user_profiles.full_name,
    ...
    successfulPayments.forEach(payment => {
      const plan = payment.payment_orders.plan_id;
    ...
    successfulPayments.forEach(payment => {
      const amount = payment.currency === 'INR' ? payment.amount / 100 : payment.amount;
      const cycle = payment.payment_orders.billing_cycle;
