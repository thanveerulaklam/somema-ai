...
export async function POST(req: NextRequest) {
  try {
    ...
    const plans = {
      starter: { name: 'Starter Plan', price: 999, plan_id_monthly: 'plan_starter_monthly', plan_id_yearly: 'plan_starter_yearly' },
      growth: { name: 'Growth Plan', price: 2499, plan_id_monthly: 'plan_growth_monthly', plan_id_yearly: 'plan_growth_yearly' },
      scale: { name: 'Scale Plan', price: 8999, plan_id_monthly: 'plan_scale_monthly', plan_id_yearly: 'plan_scale_yearly' },
    };
    ...
    const orderAmountInRupees = selectedPlan.price; // Base amount in rupees
    const orderAmount = orderAmountInRupees * 100; // Convert to paise for Razorpay
    let taxAmount = 0;
    let totalAmount = orderAmount;
    let isExport = !isIndianVisitor;

    if (isIndianVisitor) {
      taxAmount = Math.round(orderAmount * 0.18); // 18% GST on paise amount
    ...
    console.log('💰 Creating Razorpay subscription with amount:', totalAmount, 'paise (', totalAmount / 100, 'rupees) currency:', currency);
    console.log('📊 Tax calculation:', {
      baseAmountRupees: orderAmountInRupees,
      baseAmountPaise: orderAmount,
