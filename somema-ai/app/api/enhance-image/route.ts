  ...
  try {
    ...
    if (!imageResponse.ok) {
      ...
      if (err.error?.code === 'moderation_blocked' || 
          err.error?.message?.includes('safety system') ||
          err.error?.message?.includes('safety_violations')) {
        return NextResponse.json({ 
          error: 'This image cannot be enhanced due to content policy restrictions. Please try with a different image.',
