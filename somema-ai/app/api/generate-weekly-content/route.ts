...
export async function POST(request: NextRequest) {
  try {
    ...
    if (bypassCredits) {
      console.log('👑 Admin user - bypassing credit check for weekly content generation')
    }

    console.log('Generating weekly content with:', {
      ...
      try {
        ...
        for (const img of selectedImages) {
          if (img.mime_type && img.mime_type.startsWith('video/')) {
            hasVideos = true
            console.log(`🎥 Video detected in weekly content: ${img.file_name} - skipping analysis`)
            // For videos, we'll generate content directly from business profile
            // Skip the analysis step for videos
          } else {
        ...
        if (hasVideos) {
          // For videos, generate content directly from business profile (skip analysis)
          console.log('🎥 Generating video content directly from business profile for weekly post')
          
          const aiRequest = {
            businessContext: userProfile.business_name || 'our business',
            platform: 'instagram',
            theme: 'reel',
            customPrompt: `Create engaging Instagram Reel content for ${userProfile.business_name || 'our business'}. Business type: ${userProfile.industry || 'general business'}. Target audience: ${userProfile.target_audience || 'general audience'}.`,
            tone: userProfile.brand_tone || 'friendly',
            targetAudience: userProfile.target_audience || 'General audience',
            niche: userProfile.industry || 'General'
          }

          const contentResponse = await fetch(`${baseUrl}/api/generate-video-content`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${userId}`
          ...
          // For images, use the existing analysis-based approach
          ...
          generatedContent = await generateInstagramContentFromCLIP(aggregatedAnalysis, businessProfile)
        }
        ...
        generatedPosts.push({
          dayKey,
          theme: hasVideos ? 'Video Content' : (analyses.length > 0 ? 'Product' : 'Video Content'),
          caption: generatedContent.caption,
          hashtags: generatedContent.hashtags,
          imagePrompt: hasVideos ? 'Video content generated from business profile' : (analyses.length > 0 ? analyses.map(a => a.caption).join(' | ') : 'Video content'),
    ...
    console.log('Generated weekly content:', generatedPosts)
    ...
    try {
      ...
      const logs = successfulPosts.map(() => ({ user_id: userId, type: 'weekly' }))
      if (logs.length > 0) {
        await supabase.from('generation_logs').insert(logs)
      }
    } catch (logError) {
      console.error('Failed to log weekly generation:', logError)
    }
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to generate weekly content' },
      { status: 500 }
    )
  }
} 