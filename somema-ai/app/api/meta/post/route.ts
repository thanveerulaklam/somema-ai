  ...
  try {
    ...
    switch (platform) {
      ...
      case 'both':
        ...
        let facebookResult
        try {
          facebookResult = await metaService.postToFacebook(postContent)
          console.log('Facebook result:', facebookResult)
        } catch (facebookError: any) {
          console.error('Facebook posting error:', facebookError)
          facebookResult = {
            success: false,
            error: facebookError.message || 'Failed to post to Facebook'
          }
        }
    ...
    if (platform === 'both') {
      ...
      if (facebookSuccess && instagramSuccess) {
        ...
        // Check if Facebook was skipped due to file size
        if (bothResult.facebook.skipped) {
          userMessage = 'Posted to Instagram successfully! Facebook posting was skipped due to video file size limits.'
          overallSuccess = true // Still consider this a success since Instagram worked
        } else {
