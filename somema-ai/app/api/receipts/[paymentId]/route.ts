...
export async function GET(
  ...
  try {
    ...
    const orderData = payment.payment_orders?.[0];
    const userProfile = orderData?.user_profiles?.[0];

    if (!orderData || !userProfile) {
      console.error('❌ Order or user data not found');
      return NextResponse.json({ error: 'Order or user data not found' }, { status: 404 });
    }
