-- Fixed Queue System Database Schema
-- Run this in your Supabase SQL editor

-- Drop existing tables and functions if they exist
DROP TRIGGER IF EXISTS trigger_add_post_to_queue ON posts;
DROP FUNCTION IF EXISTS add_post_to_queue();
DROP FUNCTION IF EXISTS update_queue_status(UUID, TEXT, TEXT, INTEGER);
DROP FUNCTION IF EXISTS get_next_posts_to_process(INTEGER);
DROP FUNCTION IF EXISTS retry_failed_posts();
DROP TABLE IF EXISTS queue_processing_logs;
...
CREATE TABLE post_queue (
    ...
    -- Add unique constraint on post_id to prevent duplicates
    UNIQUE(post_id)
);
...
CREATE INDEX idx_queue_logs_queue_id ON queue_processing_logs(queue_id);
...
BEGIN
    ...
    IF NEW.status = 'scheduled' AND NEW.scheduled_for IS NOT NULL THEN
        ...
        ON CONFLICT (post_id) DO NOTHING; -- Now this will work because we have UNIQUE(post_id)
    END IF;
